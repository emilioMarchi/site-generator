...
interface SiteConfig {
  sitio: {
    ...
    slug: string;
    descripcion: string;
    // Tipos principales de sitio: landing, ecommerce, portfolio
    tipo: 'landing' | 'ecommerce' | 'portfolio';
    template: string;
  };
  dominio: {
    principal: string;
...
async function generateSite(
  ...
  if (typeof (overrideTipo) !== 'undefined' && overrideTipo) {
    ...
    if (fs.existsSync(candidate)) {
      ...
      if (fs.existsSync(fallbackTemplate)) templatePath = fallbackTemplate;
    }
  }
  ...
  if (!fs.existsSync(templatePath)) {
    ...
    if (effectiveTipo === 'portfolio') {
      ...
      if (fs.existsSync(portfolioPath)) {
        ...
        if (fs.existsSync(fallbackLanding)) templatePath = fallbackLanding;
      }
    } else {
